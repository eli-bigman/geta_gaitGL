...
class HESSO(BaseHybridSparseOptimizer):
    ...
    def identify_redundant_groups(self):
        ...
        if not hasattr(self, 'global_scores') or not self.global_scores:
            ...
            if (self.num_steps - self.start_pruning_step - 1) % self.pruning_period_duration == 0:
                ...
                try:
                    ...
                    if hasattr(self, 'global_scores') and len(self.global_scores) > 0:
                        ...
                        print(f"Warning: Skipping pruning at step {self.num_steps} as no importance scores were computed")
                        self.curr_pruning_period += 1
                except Exception as e:
                    print(f"Error during pruning at step {self.num_steps}: {e}")
